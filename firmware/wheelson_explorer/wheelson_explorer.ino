...
// Protocol reverse-engineered from Wheelson-Library/src/Nuvoton/WheelsonMotor.cpp
...
// Matches Wheelson-Library/src/Pins.hpp (all hardware revisions)
...
void handleStatus() {
  ...
  doc["distance_cm"] = serialized(String(dist, 1));
 ...
 * Returns: { "ok": true, "action": <actual>, "duration_ms": <int>, "distance_cm": <float> }
 */
void handleMove() {
  ...
  int    duration_ms  = doc["duration_ms"] | 500;
  float  dist         = readDistanceCM();
  ...
  resp["distance_cm"] = serialized(String(dist, 1));
  resp["safety"]      = safety_fired;
